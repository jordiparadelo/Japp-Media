import { Section, Container, Badge, WebsiteScreen } from "@/components/ui";
// import ReactEmojis from "@souhaildev/reactemojis";
import { SectionProps } from "@/types";

function BannerWebsite({ id }: SectionProps) {
	return (
		<Section id={id}>
			<Container className='flex flex-col gap-20 place-items-center sm:gap-[5vw]' style={{perspective: '1000px'}}>
				<div className='flex flex-col gap-4 place-items-center text-center'>
					<Badge
						label='🔥 Crea tu sitio Web'
					/>
					<h2 className='heading-h2'>Sitio Web Antiguo o Ineficaz</h2>
					<p className='text-lg md:text-xl leading-tight'>
						Un sitio lento y desactualizado dificulta retener clientes y ganar
						confianza.
					</p>
				</div>
				<WebsiteScreen/>
			</Container>
		</Section>
	);
}

export  {BannerWebsite};
